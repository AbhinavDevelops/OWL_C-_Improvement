...
def maxg(image):
    ...
    image_out = (image_out / np.amax(image_out)) * \
        255  # scale image between 0 - 255
    image_out = image_out.astype('uint8')
...
def gndvi(image):
    ...
    image_out = cv2.normalize(
        image_out, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    image_out = image_out.astype('uint8')
...
def dgci(image):
    ...
    imgOut = cv2.normalize(imgOut, None, alpha=0, beta=255,
...
def tenengrad_blur(image):
    ...
    gradient_magnitude = np.sqrt(np.square(sobelx) + np.square(sobely))
    blurriness = np.sum(np.square(gradient_magnitude)) / \
        (grey.shape[0] * grey.shape[1])
